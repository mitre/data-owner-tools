...
parser.add_argument(
    "--schema", nargs=1, required=True, help="Directory of linkage schema"
)
parser.add_argument(
    "--secretfile",
    nargs=1,
    required=True,
    help="Location of de-identification secret file",
)
...
for s in schema:
    ...
    subprocess.run(
        ["anonlink", "hash", source_file, secret, str(schema_path), str(output_file)]
    )
